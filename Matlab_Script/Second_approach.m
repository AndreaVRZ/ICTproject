...
%----------------------------SECOND APPROACH------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%In this second approach only open-loop data are used!
...
M05=tdsrivc(id,[1 5],'Td',0.06) %identified model with nz=0 and np=5
...
M13=tdsrivc(id,[2 3],'Td',0.06) %identified model with nz=1 and np=3
...
M15=tdsrivc(id,[2 5]) %identified model with nz=1 and np=5
...
M24=tdsrivc(id,[3 4]) %identified model with nz=2 and np=4
e24=cv.y-simc(M24,cv); %prediction error
J(11)=sum(e24'*e24) %performance index

M25=tdsrivc(id,[3 5]) %identified model with nz=2 and np=5
e25=cv.y-simc(M25,cv); %prediction error
J(12)=sum(e25'*e25) %performance index
%--------------------------------------------------------------------------
M34=tdsrivc(id,[4 4]) %identified model with nz=3 and np=4
e34=cv.y-simc(M34,cv); %prediction error
J(13)=sum(e34'*e34) %performance index

M35=tdsrivc(id,[4 5]) %identified model with nz=3 and np=5
